...
public class dataS {
    final private static List<List> arrayDatasantri = new ArrayList<>();
    final private static  Scanner input = new Scanner(System.in);
    private static int pilihan;

    public static void Main(String[] args) {
        datasantri data = null; 
        
        datasantri.display();
        
        
        data.pendataanDatasantri();
        data.jumlahhafalan();
        data.updateGrade();
    ...
    private static void jumlahhafalan(){
        if(arrayDatasantri.size() < 1){
            ...
            System.out.println("Jumlah Hafalan rata-rata santri adalah " + juz);
        }
    }

    private static void updateGrade(){
        if(arrayDatasantri.size() < 0){
            System.out.println("Belum ada data santri yang dimasukkan");
            return;
        }else{
            System.out.print("Masukkan nis santri : ");
            String nisSantri = input.next();
            for(int i = 1; i < arrayDatasantri.size(); i++){
                List<String> arrayInput = new ArrayList<>();
